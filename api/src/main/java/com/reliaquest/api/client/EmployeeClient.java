...
public class EmployeeClient {
    ...
    private boolean handleRestErrorDelete(RestClientResponseException e, String id) {
        ...
        if (status == null) {
            log.error("Unknown HTTP status code {} for delete operation on ID: {}",
                    e.getRawStatusCode(), id);
            return false;
        }
    ...
    private boolean handleRestErrorDeleteByName(RestClientResponseException e, String name) {
        HttpStatus status = HttpStatus.resolve(e.getRawStatusCode());
        if (status == null) {
            log.error("Unknown HTTP status code {} for delete operation on name: {}",
                    e.getRawStatusCode(), name);
            return false;
        }
        if (status == HttpStatus.NOT_FOUND) {
    ...
    private List<Employee> handleRestErrorList(RestClientResponseException e, String method) {
        HttpStatus status = HttpStatus.resolve(e.getRawStatusCode());
        if (status == null) {
            log.error("Unknown HTTP status code {} in {}", e.getRawStatusCode(), method);
            return Collections.emptyList();
        }
        if (isTransientError(status)) {
    ...
    private Optional<Employee> handleRestErrorOptional(
        ...
        HttpStatus status = HttpStatus.resolve(e.getRawStatusCode());
        if (status == null) {
            log.error("Unknown HTTP status code {} in {}: {}",
                    e.getRawStatusCode(), method, identifier);
            return Optional.empty();
        }
        if (status == HttpStatus.NOT_FOUND) {
